...
export const PendingRequestsProvider: React.FC<{ children: ReactNode }> = ({
	...
	try {
		...
		sqliteContext =
			dbStatus.shouldInitDb && dbStatus.isDbInitialized
				? useSQLiteContext()
				: null;
	} catch (error) {
	...
	const addActivityWithFallback = useCallback(
		(activity: PendingActivity) => {
			...
			try {
				...
				console.error(
					"Exception while trying to record activity, queuing for later:",
					error,
				);
				setPendingActivities((prev) => [...prev, activity]);
			}
	...
	const addSubscriptionWithFallback = useCallback(
		(subscription: PendingSubscription): Promise<string | undefined> => {
			if (!db) {
				console.log(
					"Database not ready, queuing subscription for later recording",
				);
			...
			try {
				...
				console.error(
					"Exception while trying to record subscription, queuing for later:",
					error,
				);
				setPendingSubscriptions((prev) => [...prev, subscription]);
			}
	...
	const getByType = useCallback(
		(type: PendingRequestType) => {
			return Object.values(nostrService.pendingRequests).filter(
		...
		for (const request of Object.values(nostrService.pendingRequests)) {
			if (
				...
				async function processPayment() {
					...
					try {
						preimage = await nostrService.payInvoice(
					...
					try {
						serviceName = await nostrService.getServiceName(
							paymentRequest.serviceKey,
						);
					} catch (e) {
		...
		(id: string) => {
			...
			switch (request.type) {
				...
				case "payment":
					request.result({
						status: new PaymentStatus.Pending(),
						requestId: (request.metadata as SinglePaymentRequest).content
							.requestId,
					});
					...
					try {
						...
						const amount =
							typeof (request.metadata as SinglePaymentRequest).content
								.amount === "bigint"
								? Number(
						...
						const currencyObj = (request.metadata as SinglePaymentRequest)
							.content.currency;
						if (currencyObj) {
						...
						nostrService
							...
							.then((serviceName) => {
								return addSubscriptionWithFallback({
									...
									next_payment_date: fromUnixSeconds(
										req.content.recurrence.firstPaymentDue,
									),
									recurrence_calendar:
										req.content.recurrence.calendar.inner.toCalendarString(),
									recurrence_max_payments:
										req.content.recurrence.maxPayments || null,
								});
							})
							.then((id) => {
								request.result({
									status: new RecurringPaymentStatus.Confirmed({
										...
										authorizedAmount: (request.metadata as SinglePaymentRequest)
											.content.amount,
										authorizedCurrency: (
											request.metadata as SinglePaymentRequest
										).content.currency,
										authorizedRecurrence: (
											request.metadata as RecurringPaymentRequest
										).content.recurrence,
									}),
									requestId: (request.metadata as RecurringPaymentRequest)
										.content.requestId,
								});
							});
					} catch (err) {
			...
			switch (request?.type) {
				...
				case "subscription":
					request.result({
						...
						requestId: (request.metadata as RecurringPaymentRequest).content
							.requestId,
					});
					break;
			}
