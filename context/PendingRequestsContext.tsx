			...
			if (resolver && request) {
				switch (request.type) {
					case "login":
						...
						getNostrServiceInstance().getServiceName(request.metadata.serviceKey).then((serviceName) => {
					...
					case "subscription":
						resolver({
							status: new RecurringPaymentStatus.Confirmed({
								...
								authorizedAmount: (request.metadata as SinglePaymentRequest)
									.content.amount,
								authorizedCurrency: (request.metadata as SinglePaymentRequest)
									.content.currency,
						...
						try {
							// Convert BigInt to number if needed
							const amount =
								typeof (request.metadata as RecurringPaymentRequest).content
									.amount === "bigint"
									? Number(
											(request.metadata as RecurringPaymentRequest).content
												.amount,
										)
									: (request.metadata as RecurringPaymentRequest).content
											.amount;
							...
							// Extract currency symbol from the Currency object
							let currency: string | null = null;
							const currencyObj = (request.metadata as RecurringPaymentRequest)
								.content.currency;
							if (currencyObj) {
								// If it's a simple string, use it directly
								if (typeof currencyObj === "string") {
